...
while ($row = $sql->fetch_object()) {
    ...
    $pdf->Cell(60, 15, $name, 1); // Adjusted cell height
    $pdf->Cell(30, 15, $destination, 1); // Adjusted cell height
    $pdf->Cell(70, 15, $purpose, 1); // Adjusted cell height
    $pdf->Cell(21, 15, $timedept, 1); // Adjusted cell height
    $pdf->Cell(20, 15, $esttime, 1); // Adjusted cell height
    $pdf->Cell(39, 15, $typeofbusiness, 1); // Adjusted cell height
    $pdf->Cell(20, 15, $time_returned, 1); // Adjusted cell height
    $pdf->Cell(55, 15, $confirmed_by, 1); // Adjusted cell height

    // Use MultiCell for the Remarks column to allow text wrapping
    $pdf->SetFont('Arial', '', 12); // Set a smaller font size for Remarks
    $pdf->MultiCell(50, 8, $remarks, 1, 'L'); // Adjusted cell width and left alignment
